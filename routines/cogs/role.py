...
class RSRole(commands.Cog, name="Role"):
    ...
    async def amount(self, level):
        async with sessionmaker.begin() as session:
            people = (
                await session.execute(select(Queue).where(Queue.level == int(level)))
            ).scalars()
            count = 0
    ...
    async def rsmod(self, ctx):
        ...
        if right_channel or ctx.guild.id != clubs_server_id:
            ...
            extra_embed = discord.Embed(
                color=discord.Color.blue(), description="Current Mods"
            )
            ...
            msg = await ctx.send(
                f"{ctx.author.mention}, this command can only be run in #bot-spam"
            )
            ...
            async with sessionmaker() as session:
                if mod in mods:
                    ...
                    for str_mod in mods:
                        str_mods += "**" + str_mod + "**" + ", "
                    await ctx.send(
                        f"{mod} not found in list, current available mods: {str_mods[:-2]}"
                    )
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        async with sessionmaker.begin() as session:
            ...
            if (
                payload.message_id == 858406627220258836
                or payload.message_id == external_rs_message
            ):  # Regular roles
                        ...
                        if rs_role != -1:
                            if payload.guild_id == clubs_server_id:
                                await payload.member.add_roles(
                                    discord.utils.get(
                                        payload.member.guild.roles, name=f"rs{rs_role}"
                                    )
                                )
                                await payload.member.add_roles(
                                    discord.utils.get(
                                ...
                                welcome_message = await channel.send(
                                    f"Welcome to the clubs {payload.member.mention}! You've been given the RS{rs_role} Role, and you will get pinged everytime someone joins a queue.\nIf you want to suppress these RS pings, type !rsc to hide the channels, and type !rsc again to see the channels again."
                                )
                            else:
                                server = await session.get(
                                    ExternalServer, payload.guild_id
                                )
                                rs_str = "rs" + rs_role
                                role_id = getattr(server, rs_str)
                                await payload.member.add_roles(
                                    discord.utils.get(
                                        payload.member.guild.roles, id=role_id
                                    )
                                )
                                channel = await self.find("c", payload.channel_id)
                                welcome_message = await channel.send(
                                    f"Welcome {payload.member.mention}! You've been given the RS{rs_role} Role, and you will get pinged everytime someone joins a queue of this red star level.\n If you want to stop receiving these pings, react with ❌ to remove the role."
                                )
                        else:
                            if payload.guild_id == clubs_server_id:
                ...
                payload.message_id == 858406639621898250
                or payload.message_id == external_rs_34_message
            ):  # 3/4 roles
                reaction = str(payload.emoji)
                try:
                    rs_role = self.emojis[reaction]
                    ...
                    if rs_role != -2:
                        ...
                        if rs_role != -1:
                            if payload.guild_id == clubs_server_id:
                                ...
                                channel = await self.find("c", payload.channel_id)
                                welcome_message = await channel.send(
                                    f"Welcome to the clubs {payload.member.mention}! You've been given the RS{rs_role} 3/4 Role, and you will get pinged when a queue of this red star level hits 3/4.\nIf you want to suppress these RS pings, type !rsc to hide the channels, and type !rsc again to see the channels again."
                                )
                            else:
                                server = await session.get(
                                    ExternalServer, payload.guild_id
                                ...
                                channel = await self.find("c", payload.channel_id)
                                welcome_message = await channel.send(
                                    f"Welcome {payload.member.mention}! You've been given the RS{rs_role} 3/4 Role, and you will get pinged when a queue a queue of this red star level hits 3/4.\n If you want to stop receiving these pings, react with ❌ to remove the role."
                                )
                        else:
                            if payload.guild_id == clubs_server_id:
                                for role in payload.member.roles:
                ...
                payload.message_id == 858406648041439282
                or payload.message_id == external_rs_silent_message
            ):  # silent roles
                reaction = str(payload.emoji)
                try:
                    rs_role = self.emojis[reaction]
                    ...
                    if rs_role != -2:
                        ...
                        if rs_role != -1:
                            if payload.guild_id == clubs_server_id:
                                ...
                                channel = await self.find("c", payload.channel_id)
                                welcome_message = await channel.send(
                                    f"Welcome to the clubs {payload.member.mention}! You've been given the RS{rs_role} Silent role, and you will get pinged ONLY when a queue you joined hits 4/4.\nIf you want to hide the hide the rs channels, type !rsc to hide them, and type !rsc to see them again."
                                )
                            else:
                                server = await session.get(
                                    ExternalServer, payload.guild_id
                                ...
                                channel = await self.find("c", payload.channel_id)
                                welcome_message = await channel.send(
                                    f"Welcome {payload.member.mention}! You've been given the RS{rs_role} Role, and you will get pinged ONLY when a queue you've joined hits 4/4.\nEnjoy your stay."
                                )
                        else:
                            if payload.guild_id == clubs_server_id:
                                for role in payload.member.roles:
                ...
                results = (
                    await session.execute(
                        select(Temp).where(Temp.message_id == payload.message_id)
                    )
                ).scalars()
                if len(results.all()) != 0:
                    if int(
                        (
                            await session.execute(
                                select(Temp).where(
                                    Temp.message_id == payload.message_id
                                )
                            )
                        )
                        .scalars()
                        ...
                        if str(payload.emoji) == "✅":
                            ...
                            await message.remove_reaction(
                                str(payload.emoji), payload.member
                            )
                            await message.delete()
                            await channel.send(
                                f"{payload.member.mention}, you are requed for a RS{level}! ({await self.amount(level)}/4)"
                            )
                            ...
                            await club_channel.send(
                                f"{User_leave.nickname} has left RS{level} ({count}/4)"
                            )
                            servers = (
                                await session.execute(
                            ...
                            for server in servers:
                                if server.min_rs <= User_leave.level <= server.max_rs:
                                    ...
                                    await channel.send(
                                        f"{User_leave.nickname} has left RS{User_leave.level} ({count}/4)"
                                    )
                            ...
                            channel = await self.find("c", payload.channel_id)
                            await channel.send(
                                f"{payload.member.mention} Don't touch that! That's not for you to react to 🤬🤬🤬"
                            )
