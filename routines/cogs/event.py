...
class RSEvent(commands.Cog, name="Event"):
    ...
    async def update_leaderboard(self):
        async with sessionmaker() as session:
            all_events = (
                await session.execute(select(Event).where(Event.score != 0))
            ).scalars()
            ...
            for event in all_events:
                run_stats = (
                    await session.execute(
                        select(Stats).where(Stats.run_id == event.run_id)
                    )
                ).scalars()
    ...
    async def rsstats(self, ctx, name=None):
        async with sessionmaker() as session:
            if name is None:
                users = (
                    await session.execute(
                        select(Stats).where(Stats.user_id == ctx.author.id)
                    )
                ).scalars()
            else:
                users = (
                    await session.execute(
                        select(Stats).where(Stats.user_id == int(name))
                    )
                ).scalars()
            stats = {
                "rs5": 0,
            ...
            for i in range(5, 12):
                if stats[f"rs{i}"] != 0:
                    stats_string += (
                        f"RS{i}"
                        + " " * (int(5) - int(len(f"RS{i}")))
                        + f"| {stats[f'rs{i}']}\n"
                    )
            stats_string += "```"
            if name is None:
                stats_embed = discord.Embed(
                    title=f"RS Stats for {ctx.author.display_name}",
                ...
                stats_embed = discord.Embed(
                    title=f"RS Stats for {full_user.display_name}",
                    description=stats_string,
                    color=discord.Color.green(),
                )
    ...
    async def rsinput(self, ctx, id, score):
        ...
        async with sessionmaker() as session:
            ...
            users = (
                await session.execute(select(Stats).where(Stats.run_id == id))
            ).scalars()
