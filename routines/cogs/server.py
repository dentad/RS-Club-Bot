            ...
            if server is not None:
                ...
                role_ids = [
                    (
                        ...
                        getattr(server, (rs[:2] + str(i) + rs[2:])),
                    )
                    for rs in rs_types
                    for i in range(5, 12)
                ]
            ...
            async with sessionmaker() as session:
                ...
                if server.min_rs <= int(level) <= server.max_rs:
                    ...
                    saying = (
                        "RS" + str(level)
                        if type == "all"
                        else "RS" + str(level) + " 3/4"
                        if type == "3/4"
                        else "RS" + str(level) + " Silent"
                        if type == "silent"
                        else ""
                    )
                    await ctx.send(
                        f"```{saying} pings set to {role} (yes that is the right role, this is just the id of the role)```"
                    )
                else:
                    if server.min_rs > int(level):
                        await ctx.send(
                            f"{level} is below your server's current min rs of rs{server.min_rs}. If you want to change this, run `!connect {level} {server.max_rs}`"
                        )
                    else:
                        await ctx.send(
                            f"{level} is above your server's current max rs of rs{server.max_rs}. If you want to change this, run `!connect {server.min_rs} {level}`"
                        )
        else:
            await ctx.send(
                "Specify the type of role this is, i.e. `all`, `3/4`, or `silent`"
            )
        ...
        if ctx.guild.id != clubs_server_id:
            async with sessionmaker() as session:
                ...
                if not server.show:
                    ...
                    await session.commit()
                    await ctx.send(
                        "The bot will now display everything related to queues, pinging roles, queueing, etc."
                    )
                else:
                    await ctx.send(
                        "The bot is already set to display everything. If you want to turn this off, run the `!hide` command."
                    )
        else:
        ...
        if ctx.guild.id != clubs_server_id:
            async with sessionmaker() as session:
                ...
                if server.show:
                    ...
                    await session.commit()
                    await ctx.send(
                        "The bot will no longer display anything related to queues, pinging roles, queuing, etc."
                    )
                else:
                    await ctx.send(
                        "The bot is already set to not display anything. If you want the bot to display everyihing, run the `!show` command."
                    )
        else:
            await ctx.send("You can't turn off the queue system on the clubs server 😠")
    ...
    async def close(self, ctx):
        async with sessionmaker() as session:
            ...
            if user_talking is None:
                ...
                servers = (
                    await session.execute(
                        select(Talking).where(Talking.run_id == run_id)
                    )
                ).scalars()
                ...
                for server in list_servers:
                    ...
                    await channel.send(
                        f"```The connection has been closed by {ctx.author.display_name}.```"
                    )
                servers = (
                    await session.execute(
                        select(Talking).where(Talking.run_id == run_id)
    ...
    async def on_message(self, message):
        ...
        if not message.author.bot and not (
            ...
            if active_global:
                ...
                if (
                    message.guild.id in [info[0] for info in total_info]
                    and message.author.id in [info[1] for info in total_info]
                        ...
                        for data in total_servers:
                            if data == clubs_server_id:
                                ...
                                rs_level = (
                                    await session.get(
                                        Stats,
                                        (club_server_info[1], club_server_info[3]),
                                    )
                                ).rs_level
                            ...
                            if server_id != message.guild.id:
                                ...
                                async with aiohttp.ClientSession() as webhook_session:
                                    webhook = Webhook.from_url(
                                        webhook_url,
                                        adapter=AsyncWebhookAdapter(webhook_session),
                                    )
                ...
                async with sessionmaker() as session:
                    ...
                    if server is not None:
                        ...
                        if check:  # sent in #feedback (send message to feedback server)
                            ...
                            username = "Conbon"
                            ext_server = (
                                (await session.execute(select(Feedback)))
                                .scalars()
                                .first()
                            )
                            webhook_url = (
                                await session.get(ExternalServer, ext_server.server_id)
                            ).webhook
    ...
    async def startglobal(self, ctx):
        ...
        if not server.global_chat:
            async with sessionmaker() as session:
                ...
                await session.commit()
                total_servers = (
                    await session.execute(select(ExternalServer))
                ).scalars()
                ...
                for name in server_info:
                    ...
                    await channel.send(
                        f"{ctx.guild.name} has joined global chat, current (other) servers with global chat enabled: {global_active_servers}"
                    )

        else:
            await ctx.send(
                "global chat is already enabled. To turn it off run `!stopglobal`"
            )
        ...
        if server.global_chat:
            async with sessionmaker() as session:
                ...
                for name in server_info:
                    channel = await self.find("c", name)
                    await channel.send(
                        f"{ctx.guild.name} has left global chat, current servers with global chat enabled: {global_active_servers}"
                    )
        else:
            await ctx.send(
                "global chat is already disabled. To turn it on run `!startglobal`"
            )
