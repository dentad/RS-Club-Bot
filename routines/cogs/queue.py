...
class RSQueue(commands.Cog, name="Queue"):
    def __init__(self, bot):
        ...
        self.emojis = {"1️⃣": 1, "2️⃣": 2, "3️⃣": 3, "4️⃣": 4}
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            create_option(
                name="level", description="The rs level", option_type=4, required=True
            )
        ],
    )
        ...
        if 5 <= level <= 11:
            async with sessionmaker() as session:
                ...
                if force or server.show:
                    embed = await self.print_queue(
                        ctx.guild, ctx.channel, level, queues, display=False, slash=True
                    )
                    if embed is None:
                        await ctx.send(
                            f"No RS{level} Queues found, you can start one by typing `!in {level}`",
                            hidden=True,
                        )
                    else:
                    ...
                    await ctx.send(
                        "The queueing system has been turned off on this server. If you want to turn it back on, have an admin run the `!show` command",
                        hidden=True,
                    )
        else:
            await ctx.send(
                f"RS{level} not found. specify a level between 5 and 11", hidden=True
            )
    ...
    async def amount(self, session, level):
        count = 0
        level_queues = (
            await session.execute(select(Queue).where(Queue.level == level))
        ).scalars()
    ...
    async def remove_players(self, ctx, session, level):
        ...
        # Print out the queue
        people = list(
            (await session.execute(select(Queue).where(Queue.level == level))).scalars()
        )
        servers = list(
            (
        ...
        for person in people:
            if person.server_id != clubs_server_id:
                ...
                club = 1
                clubs_string_people += (
                    await self.find("u", person.user_id)
                ).mention + " "
        ...
        for server in servers:
            ...
            if server.show and server.server_id not in connecting_servers:
                if server.min_rs <= level <= server.max_rs:
                    ...
                    if channel != -1:
                        await channel.send(
                            f"```The RS{level} queue has been filled.```"
                        )
        ...
        if self.event:
            ...
            await ctx.send(
                f"Your run id is: {rs_run_id}. Once the run is over, have one person from this queue run this command: `!rsinput {rs_run_id} <score>` (without the <>) and it will input the score into the leaderboards"
            )
        ...
        for person in queue_people:
            ...
            if person.amount < 4:
                user_enter = Stats(
                    ...
                    timestamp=time_data,
                    rs_level=person.level,
                    run_id=rs_run_id,
                )
                session.add(user_enter)
                if (
                    len(connecting_servers) > 1
                ):  # Only add them to the talking database if there's more than one total server
                    user_talking_enter = Talking(
                        run_id=rs_run_id,
        ...
        await rs_log_channel.send(
            f"RS{level} Started at {formated_date} PST \nUsers: {', '.join(print_people)}"
        )
    ...
    async def joining_queue(self, ctx, session, level):
        ...
        if count == 3:
            await channel.send(
                f"{name} joined {self.rs_ping_1more[f'RS{level}']} {self.rs_ping[f'RS{level}']} ({count}/4)"
            )
        else:
            await channel.send(
                f"{name} joined {self.rs_ping[f'RS{level}']} ({count}/4)"
            )
                ...
                if guild != -1 and channel != -1:
                    ...
                    if count == 3:
                        if role_34 is None:
                            ...
                            if role is None:
                                await channel.send(
                                    f"{name} joined {role_34.mention} ({count}/4)"
                                )
                            else:
                                await channel.send(
                                    f"{name} joined {role_34.mention} {role.mention} ({count}/4)"
                                )
                    else:
                        if role is None:
                            print_str = f"{name} joined rs{level} ({count}/4)"
                            ...
                            await channel.send(
                                f"{name} joined {role.mention} ({count}/4)"
                            )
    ...
    async def leaving_queue(self, ctx, session, level):
        ...
        await channel.send(
            f"{ctx.author.display_name} has left the RS{level} Queue ({queue_status}/4)"
        )
        # Print queues to other servers
        for server in servers:
            if server.min_rs <= level <= server.max_rs:
                ...
                if guild != -1 and channel != -1:
                    await self.print_queue(
                        session, guild, channel, level, queues, False
                    )
                    await channel.send(
                        f"{ctx.author.display_name} has left the RS{level} Queue ({queue_status}/4)"
                    )
        ...
        for queue in queues:
            ...
            if minutes == queue.length:
                ...
                message = await channel.send(
                    f"{user.mention}, still in for an RS{queue.level}? React ✅ to stay in the queue, and ❌ to leave the queue"
                )
                ...
                await the_clubs_channel.send(
                    f"{queue.nickname} has left RS{queue.level} ({count}/4)"
                )
                servers = (
                    await session.execute(
                        select(ExternalServer).where(ExternalServer.show is True)
                ...
                conditions = and_(
                    Temp.user_id == queue.user_id, Temp.level == queue.level
                )
                temps = list(
                    (await session.execute(select(Temp).where(conditions))).scalars()
                )
                for temp in temps:
                    channel = await self.find("c", temp.channel_id)
                    message = discord.utils.get(
                        await channel.history(limit=1000).flatten(), id=temp.message_id
                    )
    ...
    async def check_people(self):
        ...
        try:
            ...
            self.failed_embed.add_field(
                name="Error/Total",
                value=f"{self.error}/{self.index} -> {(self.error)/(self.index)}",
            )
            ...
            self.success_embed.add_field(
                name="Success/Total",
                value=f"{self.success}/{self.index} -> {(self.success)/(self.index)}",
            )
        ...
        if TESTING:
            async with sessionmaker() as session:
                user = Queue(
                    ...
                    user_id=809871917946634261,
                    amount=int(amount),
                    level=int(level),
                    time=int(time.time()),
                    length=25,
                    channel_id=ctx.channel.id,
                    nickname="RS Club Temp Bot",
                )
                session.add(user)
                await session.commit()
        ...
        if level is not None:
            ...
            async with sessionmaker() as session:
                ...
                if user is not None:
                    ...
                    await ctx.send(
                        f"{ctx.author.mention}, you are requeued for an RS{level}! {num_players}"
                    )
                    await session.commit()
                else:
            ...
            if ctx.guild.id == clubs_server_id:
                ...
                async with sessionmaker() as session:
                    user_queues = list(
                        (
                            await session.execute(
                                select(Queue).where(Queue.user_id == ctx.author.id)
                            )
                        ).scalars()
                    )
                    if len(user_queues) == 0:
                        await ctx.send(
                            f"{ctx.author.mention}, you aren't in any rs queues"
                        )
                        ...
                        await ctx.send(
                            f"You were found in these queues `{', '.join(levels)}`. Specify which one you'd like to refresh by adding the rs level to the end of the command."
                        )
    ...
    async def reset(self, ctx, level=None):
        async with sessionmaker.begin() as session:
            if level is None:
                ...
                if count > 3:
                    await session.execute(
                        delete(Queue).where(Queue.level == int(level))
                    )
    ...
    async def corp(self, ctx, *corp):
        ...
        await ctx.send(
            f"{ctx.author.display_name}, Your corp has been set to {' '.join(corp)}"
        )
        ...
        async with sessionmaker.begin() as session:
            ...
            for temp in temps:
                if temp.timestamp <= int(time.time()):
                    ...
                    if channel != -1:
                        message = discord.utils.get(
                            await channel.history(limit=1000).flatten(),
                            id=temp.message_id,
                        )
                        if message is not None:
                            await message.delete()
    ...
    @commands.command(
        name="1", help="Type +1/-1, which will add you/remove you to/from a RS Queue"
    )
    async def _one(self, ctx, level=None, length=25):
        if ctx.message.content[0] == "+":
            await self.entering_queue_basic_checks(
                ctx, level, length, ctx.message.content[1]
            )
    ...
    @commands.command(
        name="2",
        help="Type +2/-2, which will add you/remove you and another person to/from a RS Queue",
    )
    ...
    @commands.command(
        name="3",
        help="Type +3/-4, which will add you/remove you and 2 other people to/from a RS Queue",
    )
    ...
    async def entering_queue_basic_checks(
        self, ctx, level, length, count, only_allow_one=False
    ):
        async with sessionmaker.begin() as session:
            able_to_join = False
                    ...
                    if level != -1:
                        ...
                        for role in ctx.author.roles:
                            if str(role)[2:].isnumeric():  # Checks main role (rs#)
                                ...
                                2:-12
                            ].isnumeric():  # Checks 3/4 role (rs# 3/4 1more)
                                ...
                                2:-2
                            ].isnumeric():  # Checks silent role (rs# s)
                        ...
                        if has_right_role:
                            ...
                            await ctx.send(
                                f"{ctx.author.mention}, you aren't RS{level}"
                            )
                    else:
                ...
                # Command was run outside of The Clubs Server (external)
                    ...
                    if server is not None:
                            ...
                            if server.channel_id == ctx.channel.id:
                                if level is None:
                                    ...
                                    if len(confirmed_roles) == 0:
                                        await ctx.send(
                                            "You have no roles matching any known rs levels on this server."
                                        )
                                    else:
                                    ...
                                    level_34_role = getattr(
                                        server, "rs" + str(level) + "_34"
                                    )
                                    level_silent_role = getattr(
                                        server, "rs" + str(level) + "_silent"
                                    )
                                    if (
                                        level_role is not None
                                        or level_34_role is not None
                                        or level_silent_role is not None
                                    ):
                                        role_ids = []
                                        role_types = ["rs", "rs_34", "rs_silent"]
                                        for role in role_types:
                                        ...
                                        await ctx.send(
                                            f"RS{level} role not set up on this server. Set it with the `!level` command."
                                        )
                            # Command not run in the server's queue channel
                            else:
                                msg = await ctx.send(
                                    "Command not run in your server's queueing channel"
                                )
                                await asyncio.sleep(10)
                                await ctx.message.delete()
                                await msg.delete()
                        ...
                        message = await ctx.send(
                            "The bot has not been configured for queueing yet, run `!help e` and follow the directions"
                        )
            ...
            if able_to_join:
                await self.queue_further_checks(
                    ctx,
                    session,
                    entering_queue_level,
                    length,
                    count,
                    run_from_external,
                    only_allow_one,
    ...
    async def queue_further_checks(
        self, ctx, session, level, length, count, external, only_allow_one
    ):
        ...
        if allowed_to_join:
            ...
            if current_queue_status + count > 4:
                await ctx.send(
                    f"{ctx.author.mention}, the queue is currently at {current_queue_status}/4, adding {count} would overfill the queue"
                )
            else:
    ...
    async def removing_from_queue_check(self, ctx, level, amount):
        ...
        async with sessionmaker.begin() as session:
            ...
            if len(user_queues) == 0:  # Not found in any queues
                await ctx.send(
                    f"{ctx.author.mention}, You aren't in any RS Queues at the moment"
                )
                        ...
                        if server.show:
                            ...
                            if server.channel_id == ctx.channel.id:
                                ...
                                if level is None:
                                    ...
                                    if server.channel_id == ctx.channel.id:
                                        levels_of_current_queues = [
                                            str(queue.level) for queue in user_queues
                                        ]
                                        if len(user_queues) > 1:
                                            ...
                                            await self.remove_from_queue(
                                                ctx,
                                                session,
                                                levels_of_current_queues[0],
                                                amount,
                                            )
                                # Remove them with the level they want
                                else:
                                    await self.remove_from_queue(
                                        ctx, session, level, amount
                                    )
    ...
    @commands.command(
        aliases=["q"], help="(!queue or !q) will show the current RS Queues"
    )
                ...
                if server != -1 and server.show:
                    if level is None:
                        ...
                        for i in range(5, 12):
                            print_queues.append(
                                await self.print_queue(
                                    session, ctx.guild, ctx.channel, i, display=False
                                )
                            )
                        printed = False
                        ...
                        await self.print_queue(
                            session, ctx.guild, ctx.channel, int(level)
                        )
                else:
                    msg = await ctx.send(
                        "The queueing system has been turned off on this server. If you want to turn it back on, have an admin run the `!show` command"
    ...
    async def print_queue(
        self, session, guild, channel, level, display=True, slash=False
    ):
        ...
        if count > 0:
            ...
            for person in people:
                ...
                str_people += (
                    ...
                    + rsmods[i]
                    + " 🕒 "
                    + str(int((time.time() - int(person.time)) / 60))
                    + "m"
                )
                str_people += "\n"
                i += 1
            ...
            if role is not None:
                queue_embed = discord.Embed(
                    color=role.color,
                    title=f"The Current RS{level} Queue ({await self.amount(session, level)}/4)",
                    description=str_people,
                )
            else:
                queue_embed = discord.Embed(
